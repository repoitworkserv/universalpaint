                                       ...
                                       <div class="col-md-12 col-sm-12 col-sm-12 text-center">
                                          <table class="table table-striped table-condensed" style="border-collapse:collapse;">
                                             ...
                                             <tbody id="variation_tbl_body">
                                                ...
                                                @endphp
                                                <tr class="activeRow"  id="variation_Row{{$sp_count}}" data-subproduct_id="{{$sp->id}}">
                                                   @foreach($productvariable as $pv)
                                                   @foreach($variablelist as $vl)
                                                   @if($pv->variable_id == $vl->id)
                                                   <td>
                                                      <select class=" select2 select2-hidden-accessible form-control" style="width: 100%;"  name="variation_attributes[]">
                                          ...
                                          <div class="pagination"> {{ $subproduct->appends(request()->query())->links() }} </div>
                                       </div>
                                    </div>
                                 </div>
                              </div>
...
<script>
   ...
   function fn_delete_row() {
   	$('.delete_details').on('click',function(){
         ...
         product_id = $('#'+myrow).data('subproduct_id');
         attr_id    = $('#'+myrow).find('select').first().val();
         ...
         if(confirm("Are you sure you want to delete the variation?")) {
            $.ajax({
               url: "{{URL::to('/admin/product-delete-variation')}}",
               method: 'DELETE',
               dataType: 'json',
               data: data,
               success: function (data) {
