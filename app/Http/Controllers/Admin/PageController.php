...
class PageController extends Controller
{
    ...
    {	
		...
		$Home       = Post::where('post_type', 'page')->where('post_name', 'Home')->get();
		$id = Auth::id();
    ...
    { 
		...
		$PostMetaData = PostMetaData::where('meta_key','footer_left_col')->orWhere('meta_key','footer_right_col')->orWhere('meta_key','footer_icon')->paginate(10);;
		$pageid = $id;
	    if (empty($Page)) {
			abort(404);
		}
    ...
    {
        $banner = PostMetaData::where('meta_key', 'banner')->where('source_id', 1)->first();
        if ($banner) {
    ...
    {	
		$metadata = PostMetaData::where('meta_key', $request->meta_key)->where('source_id', $request->source_id)->first();
	...
	public function lnk_store(Request $request){
        ...
        if($validator->fails()){
			...
			return redirect()->back()->with(array('status'=>$status,'msg'=>$message));
        } else {
        	$txt = 'Added';	
	...
	public function link_manage_post(Request $request){
		...
		$metavalue =( strpos($metadata->meta_value, ',') !== false ) ? explode(',',$metadata->meta_value) : array($metadata->meta_value) ;
		if(is_array($metavalue)){
			if($request->tag_lnk == 'add'){
				if(!in_array($request->post_ist,$metavalue)){
					...
					return redirect()->back()->with(array('status'=>'error','msg'=>'The Post selected is currently in the list.'));
				}
			}else{
		...
		if($metadata->save()){
			if($request->tag_lnk == 'add'){
				return redirect()->back()->with(array('status'=>'success','msg'=>'Successfully Add Post in the link'));
			}else{
	...
	public function destroyBrands($id,$mkey)
    {
        ...
        return redirect()->back()->withStatus('Item removed!');
    }
